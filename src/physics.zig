...
pub fn printparticle() !void {
    for (0..main.pointlistptrattribute.*.items.len) |i| {
        ...
        try printarray(main.pointlistptrread.items[i]);
        try printarray(main.pointlistptrwrite.items[i]);
...
fn forcecollision(ind: usize) void {
    ...
    for (0..main.pointlistptrattribute.*.items.len) |i| {
        if (i != ind) {
            ...
            const vec: [2]f32 = [2]f32{ main.pointlistptrread.*.items[i][0] - main.pointlistptrread.*.items[ind][0], main.pointlistptrread.*.items[i][1] - main.pointlistptrread.*.items[ind][1] };
            const dist = math.sqrt(math.pow(f32, vec[0], 2) + math.pow(f32, vec[1], 2));
...
fn forcedrag(ind: usize, timestep: f32) void {
...
fn velcalc(ind: usize, timestep: f32) void {
    for (0..2) |i| {
...
pub fn addforce(ind: usize, force: [2]f32) void {
    for (0..2) |i| {
...
fn setzero(ind: usize) void {
    main.pointlistptrread.*.items[ind] = .{ 0.0, 0.0 };
    main.pointlistptrwrite.*.items[ind] = .{ 0.0, 0.0 };
    main.pointlistptrattribute.*.items[ind].force = .{ 0.0, 0.0 };
    main.pointlistptrattribute.*.items[ind].velocity = .{ 0.0, 0.0 };
}

fn motion(timestep: f32) void {
    for (0..main.pointlistptrattribute.*.items.len) |index| {
        for (0..2) |i| {
            ...
            main.pointlistptrwrite.*.items[index][i] = main.pointlistptrread.*.items[index][i] + main.pointlistptrattribute.*.items[index].velocity[i] * timestep;
        }
...
test "forcedrag" {
    ...
    //test normal state
    var ref = [2]f32{ 0.0, 0.0 };
    try std.testing.expectEqualSlices(f32, &ref, &pointlist_2.items[0]);
